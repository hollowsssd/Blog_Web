...
export default function EditUserPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        const res = await axios.get(`${process.env.NEXT_PUBLIC_API_HOST}/api/user/findUser/${userId}`, {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = cookies.get("token"); //Get token from cookie
      await axios.put(`${process.env.NEXT_PUBLIC_API_HOST}/api/user/update/${userId}`, formData, {
        headers: {
